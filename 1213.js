...
//there should be a SINGLE LOOP that would do the trick -- somehow loop through all three at the same time
//either a for loop or a flatMap

//array.includes
//array.indexOf
//would it be better to do this:
// arr2.includes(val1) && arr3.includes(val1)
// rather than the extra filter?

// could put all three arrays into 1, sort it, and count each element. if it shows up 3 times, then keep it

// (i = Math.max(arr1[0], arr2[0], arr3[0]);
// i < Math.min(arr1[arr1.length - 1], arr2[arr2.length - 1], arr3[arr3.length - 1]);

// find the array with shortest length

// find the largest of first integer			
// "look for it in one of the other two; if it's found, then look for it in order in the 3rd one

// if it's not found, go to the element just larger than it
// "			

// possibly the most efficient: loop over only the elements of the shortest array; only leave it for a purpose
...
//doing it in 1 line is nice... but it's only about 25% for performance
// var arraysIntersection = function (arr1, arr2, arr3) {
//     return arr1.filter((val1) => arr2.includes(val1)).filter((val12 => arr3.includes(val12)))
// };

// this is about 5% performance
// var arraysIntersection = function (arr1, arr2, arr3) {
//     switch (Math.min(arr1.length, arr2.length, arr3.length)) {
//         case arr1:
//             return arr1.filter((val1) => arr2.includes(val1)).filter((val12 => arr3.includes(val12)))
//         case arr2:
//             return arr2.filter((val2) => arr1.includes(val2)).filter((val12 => arr3.includes(val12)))
//         default:
//         case arr3:
//             return arr3.filter((val3) => arr1.includes(val3)).filter((val13 => arr2.includes(val13)))
//     }
// };


// 38% performance; 6% memory
// var arraysIntersection = function (arr1, arr2, arr3) {
    // arr1.push(...arr2, ...arr3)
    // arr1.sort()
    // return arr1.slice(0, arr1.length - 3).flatMap((val, idx, arr) => {
    //     if (val === arr1[idx + 1] && val === arr1[idx + 2]) {
    //         return [val]
    //     }
    //     return []
    // }).sort((a,b) => a-b)
...
//52% performance; 8% memory
// var arraysIntersection = function (arr1, arr2, arr3) {
//     arr1.push(...arr2, ...arr3)
//     arr1.sort()
//     return arr1.slice(0, arr1.length - 3).filter((val, idx) => 
//         (val === arr1[idx + 1]) && (val === arr1[idx + 2]) 
//     ).sort((a,b) => a-b)
// };


//this begins with the largest value of the shortest array
// var arraysIntersection = function (arr1, arr2, arr3) {
//     x = [arr1, arr2, arr3].sort((a, b) => (a.length - b.length))
//     switch (Math.max(x[0][0], x[1][0], x[2][0])) {
//         case x[0][0]:
//             while 
//         case x[1][0]:
//         case x[2][0]:
...
console.log(arraysIntersection(
